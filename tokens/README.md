...
| `joe-tokens-legacy.json` | Legacy-format copy for Token Studio sync. Generated by `npm run tokens:build` (and `tokens:legacy`). Contains **only** `primitives` and `semantic` (no `$metadata` or `$themes`), with references as `{primitives.colors.white}` etc., matching the working two-collection structure. **Do not edit**; commit so designers can sync from it. |
